  ...
  <section className={`${styles.flexCenter} ${styles.paddingY} flex-col`}>
    <div className={`${styles.flexStart} md:flex-row flex-col mb-8 w-full`}>
      <div className='flex-1 flex flex-col justify-start mr-10'>
        <a href="#home">
          <img src={logo} alt="hoobank" className='w-[266px] -h[72px] object-contain cursor-pointer'/>
        </a>
        <p className={`${styles.paragraph} mt-5 max-w-[310px]`}>
          Cara baru melakukan berbagai transaksi dengan mudah, cepat dan aman.</p>
      </div>
    ...
    <div className='w-full flex justify-between items-center md:flex-row flex-col pt-6 border-t-[1px] border-t-[#3F3E45]'>
      <p className='font-poppins font-normal text-center text-[18px] leading-[27px] text-white'>
        © 2025 <a href="#home" className='hover:text-secondary hover:underline'>HooBank.</a> All Rights Reversed.</p>
      <div className="flex flex-row md:mt-0 mt-6">
        {socialMedia.map((social, index) => (
          <img key={social.id} src={social.icon} alt={social.id} 
          className={`w-[21px] h-[21px] object-contain cursor-pointer ${index !== socialMedia.length - 1 ? "mr-6" : "mr-0"} hover:scale-110 transTime-5`}/>
        ))}
